...
static DataGhost void_dataghost = DataGhost{ Vector{ 100000, 100000, 100000 }, Vector{ 0, 0, 0 }, 0, false };
...
void NetworkManager::CheckConnection()
{
    ...
    // Clients can not change model or color
    col = { 255,
        255,
        255 };
    ...
    // Config++ seeker logic
    this->ExecuteCommmand(client, "sar_on_load sar_expand hwait $countdown cond ?do_countdown=1 \" fadein; cl_forwardspeed 175; cl_backspeed 175; cl_sidespeed 175; svar_set do_countdown 0\"");
    this->ExecuteCommmand(client, "sar_on_load cond \"?do_countdown=1\" \"fadeout 0; cl_forwardspeed 0; cl_backspeed 0; cl_sidespeed 0\"");

    // Set seeker without announcing
    this->ExecuteCommmand(client, "ghost_draw_through_walls 2; ghost_sync_countdown 0; svar_set do_countdown 0");
...
void NetworkManager::Treat(sf::Packet& packet, unsigned short udp_port)
{
    ...
    case HEADER::MESSAGE: {
        ...
        if (client) {
            ...
            if (message.rfind("/", 0) == 0) {
                bool is_admin = this->IsClientAdmin(*client);
                auto words = split(message, ' ');

                if (words[0] == "/help") {
                    this->ServerMessage(*client, "Commands:");
                    this->ServerMessage(*client, "/set_state <name> <seeker, hider, tagged> - Sets a player's state");
                    this->ServerMessage(*client, "/new_ronud <map name> <seeker countdown (seconds)> <seekers...> - Starts a new round");
                    this->ServerMessage(*client, "/found - Mark yourself as found as a hider");
                } else if (words[0] == "/set_state") {
                    if (!is_admin) {
                        this->ServerMessage(*client, "You are not allowed to run this command!");
                        break;
                    }

                    if (words.size() < 3) {
                        this->ServerMessage(*client, "Expected: /set_state <name> <seeker, hider, tagged>");
                        break;
                    }

                    auto targets = this->GetPlayerByName(words[1]);
                    if (targets.size() == 0) {
                        this->ServerMessage(*client, "Player not found");
                        break;
                    }

                    auto target = targets[0];
                    auto new_state = words[2];
                    STATE new_state_enum;

                    if (new_state == "hider") {
                        new_state_enum = STATE::HIDER;
                    } else if (new_state == "seeker") {
                        new_state_enum = STATE::SEEKER;
                    } else if (new_state == "tagged") {
                        new_state_enum = STATE::TAGGED;
                    } else {
                        this->ServerMessage(*client, "Invalid state, expected: seeker, hider, tagged");
                        break;
                    }

                    this->SetState(*client, new_state_enum);
                } else if (words[0] == "/new_round") {
                    if (!is_admin) {
                        this->ServerMessage(*client, "You are not allowed to run this command!");
                        break;
                    }
                    ...
                    if (words.size() < 4) {
                        this->ServerMessage(*client, "Expected: /new_round <map name> <seeker countdown (seconds)> <seekers...>");
                        break;
                    }

                    auto map_name = words[1];
                    int countdown;

                    try {
                        countdown = stoi(words[2]);
                    } catch (...) {
                        this->ServerMessage(*client, "Invalid seeker countdown");
                        break;
                    }

                    std::string seekers_message = "Seekers:";
                    for (size_t i = 3; i < words.size(); i++) {
                        auto targets = this->GetPlayerByName(words[i]);
                        for (auto client : targets) {
                            this->SetState(*client, STATE::SEEKER);
                            seekers_message += " " + client->name;
                        }
                    }

                    GHOST_LOG(seekers_message);

                    std::string command = "svar_set countdown " + std::to_string(countdown * 60) + "; map " + map_name;
                    this->ServerMessage(seekers_message.c_str());
                    this->ExecuteCommmandAll(command);
                } else if (words[0] == "/found") {
                    if (client->state != STATE::HIDER) {
                        this->ServerMessage(*client, "You are not a hider!");
                        break;
                    }

                    this->SetState(*client, STATE::TAGGED);
                } else {
                    this->ServerMessage(*client, "Unknown command, see /help");
                }
            } else {
    ...
    case HEADER::COLOR_CHANGE: {
        auto client = this->GetClientByID(ID);
        if (client) {
            this->ServerMessage(*client, "You are not allowed to change your color");
        }
        break;
    }
    case HEADER::UPDATE: {
...
void NetworkManager::ServerMessage(Client& client, const char* msg)
{
    GHOST_LOG(std::string("[server message (" + client.name + ")] ") + msg);
    sf::Packet packet;
    packet << HEADER::MESSAGE << sf::Uint32(0) << msg;
    client.tcpSocket->send(packet);
}

bool NetworkManager::IsClientAdmin(Client &client) {
    return std::find(this->admins.begin(), this->admins.end(), client.ID) != this->admins.end();
}

void NetworkManager::ExecuteCommmandAll(std::string cmd) {
    for (auto& client : this->clients)
        this->ExecuteCommmand(client, cmd);
}

void NetworkManager::SetState(Client& client, STATE new_state) {    
    client.state = new_state;

    std::string message = client.name + " is now ";

    switch (new_state) {
    case STATE::SEEKER:
        message += "a seeker";
        this->ExecuteCommmand(client, "ghost_draw_through_walls 0; sar_toast_create ghost You're a seeker!; svar_set do_countdown 1");
        client.color = Color{ 255, 0, 0 };
        break;
    case STATE::HIDER:
        message += "a hider";
        this->ExecuteCommmand(client, "ghost_draw_through_walls 2; sar_toast_create ghost You're a hider!; ghost_sync_countdown 0; svar_set do_countdown 0");
        client.color = Color{ 0, 255, 4 };
        break;
    case STATE::TAGGED:
        message += "tagged";
        this->ServerMessage((client.name + " is out!").c_str());
        this->ExecuteCommmand(client, "ghost_draw_through_walls 2; sar_toast_create ghost You were found!; ghost_sync_countdown 0; svar_set do_countdown 0");
        client.color = Color{ 255, 255, 255 };
        break;
    }

    GHOST_LOG(message);

    sf::Packet color_change_packet;
    color_change_packet << HEADER::COLOR_CHANGE << client.ID << client.color;
    for (auto& other : this->clients) {
        other.tcpSocket->send(color_change_packet);
    }
}

void NetworkManager::ExecuteCommmand(Client& client, std::string cmd) {
    sf::Packet packet;
    packet << HEADER::COUNTDOWN << sf::Uint32(0) << sf::Uint8(0) << sf::Uint32(0) << cmd << "";
    client.tcpSocket->send(packet);
}
...
void NetworkManager::RunServer()
{
    ...
    while (this->isRunning) {
        ...
        if (now > lastUpdate + std::chrono::milliseconds(50)) {
            ...
            sf::Packet all_packet;
            sf::Packet seeker_packet;
            all_packet << HEADER::UPDATE << sf::Uint32(0) << sf::Uint32(this->clients.size());
            seeker_packet << HEADER::UPDATE << sf::Uint32(0) << sf::Uint32(this->clients.size()); // I don't think this causes any issues

            for (auto& client : this->clients) {
                all_packet << sf::Uint32(client.ID) << client.data;
                seeker_packet << (client.state == STATE::TAGGED) ? void_dataghost : client.data;
            }

            for (auto &client : this->clients) {
                // Send appropriate packet so seekers can not see tagged people
                auto packet = client.state == STATE::SEEKER ? seeker_packet : all_packet;
...
// implement visibility logic, more chat commands, ability to add admins.